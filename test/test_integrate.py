...
class TestSolveOde:
    ...
    def test_invalid_signature(self, method):
        with pytest.raises(ValueError, match="'f' must be callable"):
            integrate.solve_ode("ode", [0], [0, 1], method, 0.1)
        with pytest.raises(ValueError, match="'f' has an invalid signature"):
            integrate.solve_ode(lambda t, y, z: y, [0], [0, 1], method, 0.1)
    ...
    def test_invalid_t_span(self, method):
        with pytest.raises(ValueError, match="Invalid values for 't_span'"):
            integrate.solve_ode(dummy_ode, [0], t_span=(1, 0), h=0.1, method=method)
        with pytest.raises(ValueError, match="Invalid values for 't_span'"):
            integrate.solve_ode(dummy_ode, [0], t_span=(0, 1, 2), h=0.1, method=method)

    @ALL_METHODS
    def test_zero_ode(self, method):
        res = integrate.solve_ode(zero_ode, np.ones(10), [0, 5], method=method, h=1e-1)
